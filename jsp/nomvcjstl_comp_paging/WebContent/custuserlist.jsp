...
<jsp:setProperty name="paging" property="countPerPage" value="5"/>
<jsp:setProperty name="paging" property="countPerBlock" value="3"/>
<sql:transaction dataSource="${sessionScope.oracleDS}">
<sql:query var="count" >
select COUNT(id) numb from CUSTUSER
</sql:query>
<fmt:parseNumber var="nums" 
               value="${count.rowsByIndex[0][0]}" type="number"/>
<c:set target='${paging}' property="totalCount" value="${nums}"/>
...
<!-- varStatus="status"를 이용하여 
         숫자 파악 자동으로 0부터 인덱스를 부여한다 -->
...
<input type='checkbox' name='delck' value='${cust.id}'/></td>
		   <td>${cust.id}</td>
		  <td> <a href='custuserdetail.jsp?id=${cust.id}'>
		                               ${cust.name}</a></td>
		</tr>
