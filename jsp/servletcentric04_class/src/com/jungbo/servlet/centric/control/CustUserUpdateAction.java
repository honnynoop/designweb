...
public class CustUserUpdateAction implements Action {
	...
	public ActionForward execute(
		...
		System.out.println("CustUserUpdateAction : "+id+"/"+name+"/"+address);
		int count=0;
		ActionForward forword=new ActionForward();
		if(isNull(id) ||isNull(name)||isNull(address) ){
			count=0;
		}else{
			count=manager.updateCustUser(new CustUserDto(id,name,address));
		}
		
		if(count>0){//
			//update된 데이터 가져오기
			CustUserDto cust= manager.getCustUser(id);
			System.out.println("CustUserUpdateAction : "+cust);
			//request 스코프 객체에 담기
			...
			errors.setErrorType("DB관련, 고객 정보 변경중에 실패하였습니다. 없는 아이디인가를 확인하십시오. ");
