...
public class CustUserManager extends JdbcDaoSuport implements ICustUserManager {
	...
	public List<CustUserDto> getCustUserList(int startn, int endn) {
        Connection conn = null;
		  ...
		  ResultSet rs = null;
		  String strQuery = " SELECT ID,NAME,ADDRESS FROM " +
		  ...
		  try {
			...
			while(rs.next()){
			 ...
			 System.out.println("[getCustUserList : Exception ]" + ex);
		  }finally{
	...
	public int getCount() {
		...
		PreparedStatement  stmt = null;
		ResultSet rs = null;
		String strQuery = "SELECT COUNT(*)  FROM  CUSTUSER";
